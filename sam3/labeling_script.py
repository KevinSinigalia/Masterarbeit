...
file_name = "tracking_results_video_001_0001_to_0040.pkl.gz"
# Eingabe-Ordner & -Datei
INPUT_DIR = "./outputs_raw_masks_compressed"
INPUT_DIR = "./add_missing_objects"
...
# Extrahiere gespeicherte Kategorien, falls die Datei schon einmal bearbeitet wurde
saved_categories = {}
for cat_key in ["snail", "fish", "fish_other_tank", "reflection", "dirt"]:
    if cat_key in data:
        saved_categories[cat_key] = data[cat_key]

# Nur numerische Frame-Keys filtern
raw_frame_keys = sorted([k for k in data.keys() if isinstance(k, int)])
selected_keys = [k for k in raw_frame_keys if START_FRAME <= k <= END_FRAME]

outputs_per_frame = {k: data[k] for k in selected_keys}
...
for f_idx in list(outputs_per_frame.keys()):
...
# ==============================================================================
# POP-UP DIALOG: ID-KATEGORISIERUNG
# ==============================================================================
class CategorizeIDsDialog(tk.Toplevel):
    def __init__(self, parent, sorted_ids, current_mapping):
        super().__init__(parent)
        self.title("🏷️ IDs Kategorisieren")
        self.geometry("480x520")
        self.resizable(False, False)

        self.parent = parent
        self.sorted_ids = sorted_ids
        self.mapping = copy.deepcopy(current_mapping)
    ...
    def _build_ui(self):
        ttk.Label(
            self,
            text="Ordne allen erkannten IDs eine eindeutige Kategorie zu:\n(Eine ID kann jeweils nur einer Kategorie angehören)",
            font=("Arial", 9, "bold"),
            justify=tk.CENTER
        ...
        for obj_id in self.sorted_ids:
            row_frame = ttk.Frame(scrollable_frame)
            row_frame.pack(fill=tk.X, pady=4, padx=5)

            lbl = ttk.Label(row_frame, text=f"Objekt ID {obj_id}:", width=15, font=("Arial", 9, "bold"))
            lbl.pack(side=tk.LEFT, padx=5)

            cb = ttk.Combobox(row_frame, values=self.CATEGORIES, state="readonly", width=22)
            current_cat = self.mapping.get(obj_id, "Unkategorisiert")
            if current_cat in self.CATEGORIES:
                cb.set(current_cat)
            else:
                cb.set("Unkategorisiert")

            cb.pack(side=tk.LEFT, padx=5)
            self.comboboxes[obj_id] = cb

        btn_frame = ttk.Frame(self)
        btn_frame.pack(side=tk.BOTTOM, fill=tk.X, pady=10)

        ttk.Button(btn_frame, text="✅ Speichern & Übernehmen", command=self._on_save).pack(side=tk.RIGHT, padx=15)
        ttk.Button(btn_frame, text="Abbrechen", command=self.destroy).pack(side=tk.RIGHT, padx=5)

    def _on_save(self):
        for obj_id, cb in self.comboboxes.items():
            val = cb.get()
            self.mapping[obj_id] = val
        self.result_mapping = self.mapping
        self.destroy()


# ==============================================================================
# FENSTER 1: INITIALER VIEWER & ID-AUSWAHL UND ID-LÖSCHUNG / KATEGORISIERUNG
# ==============================================================================
class InitialViewerWindow(tk.Tk):

    def __init__(self, video_frames, outputs_per_frame, saved_categories, overlay_masks_func):
        super().__init__()
        self.title("Fenster 1: Masken-Vorschau & ID-Auswahl / ID Löschen / Kategorisierung")
        ...
        # Nur Ganzzahlen-Indices als Frame-Keys akzeptieren!
        self.frame_keys = sorted([k for k in self.outputs_per_frame.keys() if isinstance(k, int)])
        self.total_frames = len(self.frame_keys)
        self.current_frame_idx = 0
        self.selected_ids = []

        # Mapping für Objekt-Kategorien: {obj_id: "category_name"}
        self.id_category_mapping = {}
        self._init_categories_from_saved(saved_categories)
    ...
    def _init_categories_from_saved(self, saved_categories):
        """Lädt bestehende Kategorisierungen aus vorherigem Speichern zurück."""
        if not saved_categories:
            return
        for oid in saved_categories.get("snail", []):
            self.id_category_mapping[int(oid)] = "snail"

        fish_dict = saved_categories.get("fish", {})
        if isinstance(fish_dict, dict):
            for subcat in ["wild_type", "golden_type", "casper_type", "longfin_type"]:
                for oid in fish_dict.get(subcat, []):
                    self.id_category_mapping[int(oid)] = subcat

        for cat in ["fish_other_tank", "reflection", "dirt"]:
            for oid in saved_categories.get(cat, []):
                self.id_category_mapping[int(oid)] = cat

    def _refresh_detected_ids(self):
        """Sucht alle aktuell vorhandenen IDs in allen Frames."""
        self.all_detected_ids = set()
        for f_idx, f_data in self.outputs_per_frame.items():
            if isinstance(f_idx, int) and isinstance(f_data, dict) and "out_obj_ids" in f_data and f_data[
    ...
    def get_categories_structure(self):
        """Erstellt die gewünschte Dict-Struktur für die Kategorien."""
        cat_dict = {
            "snail": [],
            "fish": {
                "wild_type": [],
        ...
        for obj_id, category in self.id_category_mapping.items():
            if category == "snail":
                cat_dict["snail"].append(obj_id)
            elif category == "wild_type":
                cat_dict["fish"]["wild_type"].append(obj_id)
            elif category == "golden_type":
                cat_dict["fish"]["golden_type"].append(obj_id)
            elif category == "longfin_type":
                cat_dict["fish"]["longfin_type"].append(obj_id)
            elif category == "casper_type":
                cat_dict["fish"]["casper_type"].append(obj_id)
            elif category == "fish_other_tank":
                cat_dict["fish_other_tank"].append(obj_id)
            elif category == "reflection":
                cat_dict["reflection"].append(obj_id)
            elif category == "dirt":
                cat_dict["dirt"].append(obj_id)
    ...
    def _build_ui(self):
        ...
        # FILTER-ZEILE
        filter_frame = ttk.Frame(action_container)
        filter_frame.pack(side=tk.TOP, fill=tk.X, pady=2)

        ttk.Label(
            filter_frame,
            text="👁️ Masken filtern (Nur Typ anzeigen):",
            font=("Arial", 9, "bold"),
            foreground="blue"
        ).pack(side=tk.LEFT, padx=5)
        ...
        self.ids_entry = ttk.Entry(id_edit_frame, width=25)
        ...
        categorize_btn = ttk.Button(
            id_edit_frame,
            text="🏷️ IDs Kategorisieren",
            command=self._on_open_categorize_dialog,
        )
    ...
    def _get_filtered_frame_data(self, key):
        """Filtert die Masken des aktuellen Frames basierend auf dem ausgewählten Filter im Dropdown."""
        selected_filter = self.filter_combobox.get()
        raw_data = self.outputs_per_frame.get(key, {})

        if selected_filter == "Alle" or not isinstance(raw_data, dict):
            return raw_data

        if "out_obj_ids" not in raw_data or raw_data["out_obj_ids"] is None:
            return raw_data

        orig_ids = raw_data["out_obj_ids"]
        orig_masks = raw_data.get("out_binary_masks", None)
        ...
        fish_subcategories = {"wild_type", "golden_type", "casper_type", "longfin_type", "fish"}
        all_defined_categories = fish_subcategories | {"snail", "fish_other_tank", "reflection", "dirt"}

        filtered_dict = {}
        for idx, obj_id in enumerate(ids_list):
            category_of_id = self.id_category_mapping.get(obj_id, "Unkategorisiert")

            match = False
            if selected_filter == "fish":
                if category_of_id in fish_subcategories:
                    match = True
            elif selected_filter == "Unkategorisiert":
                # Trifft zu, wenn explizit 'Unkategorisiert' oder keine der definierten Kategorien zugewiesen ist
                if category_of_id == "Unkategorisiert" or category_of_id not in all_defined_categories:
                    match = True
            else:
                if category_of_id == selected_filter:
                    match = True

            if match and idx < len(masks_arr):
                filtered_dict[obj_id] = masks_arr[idx]

        return filtered_dict

    def _on_open_categorize_dialog(self):
        """Öffnet den Pop-up Dialog zur Zuordnung von Kategorien zu IDs."""
        dialog = CategorizeIDsDialog(self, self.sorted_detected_ids, self.id_category_mapping)
        self.wait_window(dialog)
        if dialog.result_mapping is not None:
            self.id_category_mapping = dialog.result_mapping
            self.update_display()
            messagebox.showinfo("Erfolg", "Kategorien wurden erfolgreich zugeordnet!")
    ...
    def _on_delete_id(self):
        ...
        for f_idx, frame_data in self.outputs_per_frame.items():
            if not isinstance(f_idx, int) or not isinstance(frame_data, dict):
                continue
        ...
        self.id_category_mapping.pop(target_id, None)
        self._refresh_detected_ids()
    ...
    def update_display(self):
        ...
        # WENDE NEN FILTER AN
        filtered_masks = self._get_filtered_frame_data(key)

        rendered_left = self.overlay_masks(
            self.video_frames[self.current_frame_idx],
            filtered_masks,
            alpha=0.25,
            draw_borders=True,
            show_ids=True,
        ...
        rendered_right = self.video_frames[self.current_frame_idx].copy()
...
class InteractiveRefinementWindow(tk.Tk):
    ...
    def __init__(
        ...
        self.frame_keys = sorted([k for k in self.video_segments.keys() if isinstance(k, int)])
        self.total_frames = len(self.frame_keys)
        self.current_frame_idx = 0
    ...
    def _on_create_dict(self):
        ...
        self.status_label.config(
            text="🔄 Füge korrigierte & unkorrigierte Masken sowie Kategorien zusammen... Bitte warten!"
        )
        self.update()
        ...
        try:
            ...
            # Füge die Kategorien als Metadaten / eigene Schlüssel am Kopf hinzu
            for cat_key, cat_val in self.categories_dict.items():
                new_dict[cat_key] = cat_val
            ...
            messagebox.showinfo(
                "Erfolg",
                f"Neues Dict mit den aktualisierten Masken & Kategorien wurde gespeichert unter:\n{self.output_file_path}",
            )
    ...
    def _on_start_tracking(self):
        ...
        try:
            ...
            for f_idx in sorted([k for k in self.outputs_per_frame.keys() if isinstance(k, int)]):
...
if __name__ == "__main__":
    print("📺 Öffne Fenster 1 (Masken-Vorschau)... Bitte IDs wählen und ggf. kategorisieren!")
    ...
    categories_dict = win1.get_categories_structure()

    if not ids_to_correct:
        print("❌ Keine IDs ausgewählt. Skript wird beendet.")
        sys.exit()

    print(f"🎯 Ausgewählte IDs: {ids_to_correct}")
    print(f"🏷️ Zuordnungs-Struktur der Kategorien: {categories_dict}")
